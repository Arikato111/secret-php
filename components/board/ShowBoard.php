...
$cat = $getParams();
if ($cat) {
    $cate = fetch($db->query("SELECT * FROM cat WHERE cat_path = '$cat'"));
    $allBoard = $db->query("SELECT * FROM board WHERE cat_id = {$cate['cat_id']} ORDER BY b_id DESC");
} else {
    $allBoard = $db->query("SELECT * FROM board ORDER BY b_id DESC");
}
